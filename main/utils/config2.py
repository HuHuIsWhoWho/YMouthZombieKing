...
# 高光谱专用配置
_C.DATA.HSI_DATASET = 'Indian_Pines'  
_C.DATA.TRAIN_RATIO = 0.2             
_C.DATA.VAL_RATIO = 0.1               
_C.DATA.PATCH_SIZE = 7   
_C.DATA.BANDS = 200
_C.DATA.NUM_CLASSES = 16             
...
_C.TRAIN.AUTO_RESUME = False  # 不自动resume
...
def update_config(config, args):
    ...
    # 添加高光谱相关参数检查(有必要吗？)
    if _check_args('hsi_dataset'):
        config.DATA.HSI_DATASET = args.hsi_dataset
    if _check_args('train_ratio'):
        config.DATA.TRAIN_RATIO = args.train_ratio
    if _check_args('val_ratio'):
        config.DATA.VAL_RATIO = args.val_ratio
    if _check_args('patch_size'):
        config.DATA.PATCH_SIZE = args.patch_size
    if _check_args('bands'):
        config.DATA.BANDS = args.bands
