# 初始更改自 models/vHeat.py
# 含插值HSIToImage模块
# Stem改为HSI_DWS_Stem，对空间的嵌入改成对光谱的嵌入
# Downsample改为只在光谱维度下采样
...
class StemLayer(nn.Module):
    ...
    def __init__(self,
                 in_chans=3,  # <1>：对应高光谱bands，时这一层主要起到一个光谱维度降维和空间维度特征融合的作用，但这样有个问题是参数量大，所以有了HSI_DWS_Stem
                 out_chans=96,
                 act_layer='GELU',
                 norm_layer='BN',):
        super().__init__()
...
class HSI_DWS_Stem(nn.Module):
    """高光谱专用深度可分离Stem"""
    def __init__(self, in_c=200, out_c=96, act_layer='GELU',norm_layer='BN',):
        super().__init__()
        ...
        self.pw1 = nn.Conv2d(in_c, in_c//2, 1)  # 先压缩
        self.norm2 = build_norm_layer(in_c//2, norm_layer, 'channels_first', 'channels_first')
        self.pw2 = nn.Conv2d(in_c//2, out_c, 1)  # 再扩展
        self.act = build_act_layer(act_layer)
...
class HeatBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # 把OP从Heat2D改为高光谱的空间和光谱融合的SpaSpeHeat
...
# 简单将HSI图像块变成三通道伪图像
# 具体操作：展平、插值、reshape
# 接在初始的vHeat模型最前端就能以最简单的方式使vHeat适配HSI数据
class HSIToImage(nn.Module):
    def __init__(self, hsi_band, hsi_patch_size, in_chans, img_size):
        super().__init__()
        self.hsi_band = hsi_band
        self.hsi_patchsize = hsi_patch_size
        self.in_len = hsi_band * hsi_patch_size * hsi_patch_size
        self.out_len = in_chans * img_size * img_size
        self.in_chans = in_chans
        self.img_size = img_size

    def forward(self, x):
        # x: [B, bands, patch, patch]
        B = x.shape[0]
        # ensure float32
        x = x.float()
        # flatten spatial+bands -> [B, L_in]
        flat = x.view(B, -1)            # [B, L_in]
        # make it (B, C=1, L_in) to use interpolate 1D
        flat = flat.unsqueeze(1)        # [B, 1, L_in]
        # interpolate to length out_len (1D)
        # use mode='linear' (1D) which is parameter-free
        flat_up = F.interpolate(flat, size=self.out_len, mode='linear', align_corners=False)  # [B,1,out_len]
        # remove channel dim and reshape to image
        out = flat_up.squeeze(1).view(B, self.in_chans, self.img_size, self.img_size)
        return out


class vHeat4HSI(nn.Module):
    def __init__(self, hsi_patch_size=7, hsi_band=200, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], 
        ...
        # 阶段0：简单HSI到伪图像的转换模块
        # self.hsi_mapper = HSIToImage(
        #     hsi_band=hsi_band,
        #     hsi_patch_size=hsi_patch_size,
        #     in_chans=in_chans,
        #     img_size=img_size,
        # )
        ...
        #                              act_layer='GELU',
        #                              norm_layer='LN')
        self.patch_embed = HSI_DWS_Stem(in_c=self.hsi_band, out_c=self.embed_dim)
        # 此时图像大小：hsi_patch_size x hsi_patch_size x embed_dim
